...
-- Conectar al servidor Linux via Cloudflared
local function connectToLinux()
    term.clear()
    term.setCursorPos(1, 1)
    
    print("=== Linux Terminal Client (Cloudflared) ===")
    print("")
    print("Options:")
    print("1. Use Cloudflare Tunnel URL")
    print("2. Use localhost (for testing)")
    print("")
    ...
    if option == "1" then
        print("")
        print("Enter your Cloudflare tunnel URL:")
        print("Example: https://abc-def-123.trycloudflare.com")
        write("Tunnel URL: ")
        local tunnel_url = read()
        
        if tunnel_url == "" then
            print("Error: Tunnel URL cannot be empty!")
            return false
        end
        
        -- Asegurar que tiene https://
        if not tunnel_url:match("^https?://") then
            tunnel_url = "https://" .. tunnel_url
        end
        
        LinuxTerminal.server_url = tunnel_url
    else
        write("Local host (default: localhost): ")
        local host = read()
    ...
    local init_response, error_msg = http.request({
    ...
    if not init_response then
        ...
        print("Error: " .. (error_msg or "Connection refused"))
        print("Make sure your Flask server is running on " .. LinuxTerminal.server_url)
        print("And that HTTP requests are enabled in CC settings.")
        return false
    end
...
local function sendCommand(command)
    ...
    local response, error_msg = http.request({
    ...
    if not response then
        return {error = "Connection lost: " .. (error_msg or "Unknown error")}
    end
    ...
    if success then
        ...
        return {error = "Invalid response: " .. response_data}
    end
end
